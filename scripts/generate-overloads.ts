...
const generateSingleOverload = (ascending) => (length) => {
  ...
  const fullType = `${genericsList}(${fnsList}): (initialArg: T1) => R`
...
const generateOverloads = (length, name, ascending) => {
  ...
  const defaultOverload = `(...fns: Array<(arg: any) => any>): (initialArg: any) => unknown`
...
generateOverloads(20, 'Pipe', true)
generateOverloads(20, 'Compose', false)
